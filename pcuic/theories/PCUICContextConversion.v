...
From MetaCoq.PCUIC Require Import PCUICAst PCUICOnOne PCUICAstUtils
     ...
     PCUICCumulativity PCUICReduction PCUICEquality PCUICEqualityLemmas
     PCUICParallelReduction PCUICEquality PCUICUnivSubstitutionConv
     PCUICParallelReductionConfluence PCUICConfluence
     PCUICContextReduction PCUICClosed PCUICOnFreeVars PCUICWellScopedCumulativity
...
Definition closed_red_context Σ Γ Γ' :=
  All2_fold (fun Γ _ => All_decls (closed_red Σ Γ)) Γ Γ'.

Notation "Σ ⊢ Γ ⇝ Δ" := (closed_red_context Σ Γ Δ) (at level 50, Γ, Δ at next level,
  format "Σ  ⊢  Γ  ⇝  Δ") : pcuic.

Lemma closed_red_context_red_context {Σ Γ Γ'} :
  Σ ⊢ Γ ⇝ Γ' -> red_context Σ Γ Γ'.
Proof.
  ...
  now move=> [].
Qed.
Coercion closed_red_context_red_context : closed_red_context >-> red_context.
...
Section ContextReduction.
  ...
  Local Definition red1_red_contextP Γ Γ' :=
    (forall n b b',
        option_map decl_body (nth_error Γ n) = Some (Some b) ->
        option_map decl_body (nth_error Γ' n) = Some (Some b') ->
        red_context Σ (skipn (S n) Γ) (skipn (S n) Γ') ->
        ∑ t, red Σ (skipn (S n) Γ') b t * red Σ (skipn (S n) Γ') b' t).

  Lemma red_context_skip i Γ Γ' :
    red1_red_contextP Γ Γ' ->
    red1_red_contextP (skipn i Γ) (skipn i Γ').
  Proof using Type.
    rewrite /red1_red_contextP => H n b b'.
  ...
  Lemma red1_red_contextP_app {Γ Γ' Δ} :
    red1_red_contextP Γ Γ' ->
    red1_red_contextP (Γ ,,, Δ) (Γ' ,,, Δ).
  Proof using Type.
  ...
  Lemma red_context_app Γ Γ' Δ :
    red_context Σ Γ Γ' -> red_context Σ (Γ ,,, Δ) (Γ' ,,, Δ).
  Proof using Type.
    intros h; eapply All2_fold_app => //.
    eapply All2_fold_refl. intros Δ' ?. reflexivity.
  Qed.
  Hint Resolve red_context_app : pcuic.

  Lemma red_context_on_free_vars P Γ Γ' :
    red_context Σ Γ Γ' ->
  ...
  Lemma red1_red_context_aux {Γ Γ' T U} :
    red1 Σ Γ T U ->
    wf_term_ctx Γ ->
    wf_term T ->
    red_context Σ Γ Γ' ->
  ...
  Proof using wfΣ.
    intros r onΓ onT H. revert onΓ onT Γ' H.
    simpl in *. induction r using red1_ind_all; intros; auto with pcuic;
      repeat inv_wf_term.
    all:try solve [eexists; t].
    all:try specialize (IHr ltac:(eauto with fvs) ltac:(tea)).
    ...
    - pose proof H.
      ...
      eapply nth_error_decl_body_wf_term_ctx in onΓ as onbody; tea.
      eapply wf_term_red_context in onΓ as onΓ'; tea.
      eapply nth_error_decl_body_wf_term_ctx in onΓ' as onbody'; tea.
      ...
      * etransitivity. eapply red1_red. constructor.
        ...
        rewrite -(firstn_skipn (S i) Γ') wf_term_ctx_app in onΓ'.
        now move/andP: onΓ' => [].
      * eapply weakening_red_0; eauto.
        rewrite firstn_length_le //.
        rewrite -(firstn_skipn (S i) Γ') wf_term_ctx_app in onΓ'.
    ...
    - destruct (IHr (Γ' ,,, inst_case_predicate_context p)).
      now eapply red_context_app => //.
      now eapply red1_red_contextP_app.
      destruct p0.
    ...
    - apply forallb_All in b. eapply OnOne2_All_mix_left in X; tea.
      eapply (OnOne2_exist _
        ...
        inv_wf_term.
        forward b5 by eauto with fvs.
        forward b5 by eauto with fvs.
        specialize (b5 (Γ' ,,, inst_case_branch_context p y)) as [body' [rl rr]].
        + rewrite /inst_case_branch_context -b4. now eapply red_context_app => //.
        + rewrite /inst_case_branch_context -b4. now eapply red1_red_contextP_app.
        + exists {| bcontext := bcontext x; bbody := body' |}; cbn; split; rewrite -?b;
          intuition eauto.
          rewrite /inst_case_branch_context b4 //.
    ...
    - apply OnOne2_prod_inv in X as [_ X].
      apply OnOne2_apply, OnOne2_apply_forallb in X; tas.
      eapply OnOne2_apply, OnOne2_apply, OnOne2_apply in X; tea.
      eapply (OnOne2_exist _ (red Σ Γ')) in X as [rl [l0 l1]]; auto.
      eexists; split; eapply red_evar; eauto.
      all: eapply OnOne2_All2; eauto.
    - eapply OnOne2_All_mix_left in X. 2: now eapply forallb_All.
      eapply (OnOne2_exist _ (on_Trel_eq (red Σ Γ') dtype (fun x => (dname x, dbody x, rarg x)))) in X.
      2: { intros d d' [[[_ IH] eq] [wft wfb]%andb_and].
        edestruct IH as (t' & Hred & Heq); tea.
        eexists {| dtype := t' |}; split; split; eauto. }
      destruct X as [mfix' [l r]].
      exists (tFix mfix' idx); split; eapply red_fix_ty.
      1-2: eapply OnOne2_All2; intuition eauto; intuition.
    - assert (fix_context mfix0 = fix_context mfix1) by eapply OnOne2_eq_fixcontext, X.
      eapply OnOne2_All_mix_left in X. 2: now eapply forallb_All.
      eapply (OnOne2_exist _ (on_Trel_eq (red Σ (Γ' ,,, fix_context mfix0)) dbody (fun x => (dname x, dtype x, rarg x)))) in X.
      2: { intros d d' [[[_ IH] eq] [wft wfb]%andb_and].
        edestruct IH as (b' & Hred & Heq); eauto with fvs.
        - eapply All2_fold_app; tea. apply All2_fold_over_red_refl.
        - now eapply red1_red_contextP_app.
        - eexists {| dbody := b' |}; split; split; eauto. }
      destruct X as [mfix' [l r]].
  ...
  Hint Resolve red_context_on_free_vars : fvs.

  Lemma red_red_context' {Γ : closed_context} Γ' {T : open_term Γ} {U} :
    red Σ Γ T U ->
    red_context Σ Γ Γ' ->
    red1_red_contextP Γ Γ' ->
    ∑ t, red Σ Γ' T t * red Σ Γ' U t.
  Proof using wfΣ.
    ...
    - eapply red1_red_context_aux; eauto with fvs.
    - exists x; split; auto.
    - destruct IHr1 as [xl [redl redr]]; eauto with fvs.
      destruct IHr2 as [xr [redl' redr']]; eauto with fvs.
      assert (on_free_vars_ctx xpred0 Γ'). eapply red_context_on_free_vars; tea. eauto with fvs.
  ...
  Lemma red_red_context_aux' {Γ : closed_context} {Γ'} :
    red_context Σ Γ Γ' -> red1_red_contextP Γ Γ'.
  Proof using wfΣ.
    destruct Γ as [Γ onΓ].
    intros X. cbn in *.
    ...
    - depelim X.
      ...
      + specialize (IHΓ byfvs _ X).
        ...
        * move=> b0 b1 [] <- [] <- H.
          ...
          eapply (@red_red_context' (exist Γ onΓ) _ (exist b onb)) in H; eauto.
        * simpl. eapply IHΓ.
  Qed.
  ...
  Lemma red_red_context {Γ : closed_context} {Γ'} {T : open_term Γ} {U} :
    ...
    ∑ t, red Σ Γ' T t * red Σ Γ' U t.
  Proof using wfΣ.
    intros. eapply red_red_context', red_red_context_aux'; eauto.
  Qed.
...
#[global] Hint Resolve red_context_on_free_vars : fvs.

Lemma red_context_on_free_vars_term {Σ P Γ Γ' t} :
  red_context Σ Γ Γ' ->
...
Qed.
#[global] Hint Resolve red_context_on_free_vars_term : fvs.
...
Section ContextConversion.
  ...
  Lemma fill_pb {Γ : closed_context} {pb} {t u : open_term Γ} {t' u'} :
    compare_term Σ.1 Σ Γ pb t u -> red Σ Γ t t' -> red Σ Γ u u' ->
    ∑ t'' u'', red Σ Γ t' t'' * red Σ Γ u' u'' * compare_term Σ Σ Γ pb t'' u''.
  Proof using wfΣ.
    intros tu tt' uu'.
    eapply red_eq_term_upto_univ_l in tu; try exact tt'. all:try tc.
    2-3: now eauto with fvs.
    destruct tu as [u'' [uu'' t'u'']].
    unshelve epose proof (red1_confluent uu' uu'') as [unf [ul ur]]; eauto with fvs.
  ...
  Lemma fill_le {Γ : closed_context} {t u : open_term Γ} {t' u'} :
    leq_term Σ.1 Σ Γ t u -> red Σ Γ t t' -> red Σ Γ u u' ->
    ∑ t'' u'', red Σ Γ t' t'' * red Σ Γ u' u'' * leq_term Σ Σ Γ t'' u''.
  Proof using wfΣ.
    apply fill_pb.
  Qed.

  Lemma fill_eq {Γ : closed_context} {t u : open_term Γ} {t' u'} :
    eq_term Σ.1 Σ Γ t u -> red Σ Γ t t' -> red Σ Γ u u' ->
    ∑ t'' u'', red Σ Γ t' t'' * red Σ Γ u' u'' * eq_term Σ.1 Σ Γ t'' u''.
  Proof using wfΣ.
    apply fill_pb.
  Qed.

  Lemma red_context_ws_cumul_ctx_pb {l Γ Γ'} : Σ ⊢ Γ ⇝ Γ' -> Σ ⊢ Γ ≤[l] Γ'.
  Proof using wfΣ.
    induction 1; constructor; auto.
    depelim p; constructor; eauto with fvs; pcuic.
  Qed.

  Lemma red_context_closed_left {Γ Γ'} : Σ ⊢ Γ ⇝ Γ' -> is_closed_context Γ.
  Proof using Type.
  ...
  Lemma red_context_closed_right {Γ Γ'} : Σ ⊢ Γ ⇝ Γ' -> is_closed_context Γ'.
  Proof using wfΣ.
    induction 1; simpl; auto.
    rewrite on_free_vars_ctx_snoc IHX /=.
    ...
    eauto with fvs.
  Qed.
  Hint Resolve red_context_closed_left red_context_closed_right : fvs.

  Lemma red_compare_term_l {pb Γ} {u v u' : term} :
    wf_term_ctx Γ ->
    wf_term u ->
    compare_term Σ Σ Γ pb u u' ->
    red Σ Γ u v ->
    ∑ v' : term, red Σ Γ u' v' × compare_term Σ Σ Γ pb v v'.
  Proof using wfΣ.
    destruct pb; cbn;
  ...
  Lemma closed_red_compare_term_l {pb Γ} {u v u' : term} :
    ...
    compare_term Σ Σ Γ pb u u' ->
    Σ ;;; Γ ⊢ u ⇝ v ->
    ∑ v' : term, Σ ;;; Γ ⊢ u' ⇝ v' × compare_term Σ Σ Γ pb v v'.
  Proof using wfΣ.
    intros isop comp [clΓ clu red].
    unshelve epose proof (red_compare_term_l _ _ comp red) as [nf [r eq]].
    1-2: eauto with fvs.
  ...
  Proof using wfΣ.
    intros isop comp [clΓ clu red].
    unshelve epose proof (red_compare_term_r _ _ comp red) as [nf [r eq]].
    1-2: eauto with fvs.
    exists nf; repeat (split; eauto with fvs).
  Qed.

  Lemma closed_red_red_context {Γ Γ'} {T U} :
    Σ ⊢ Γ ⇝ Γ' ->
    Σ ;;; Γ ⊢ T ⇝ U ->
    ∑ t, Σ ;;; Γ' ⊢ T ⇝ t × Σ ;;; Γ' ⊢ U ⇝ t.
  Proof using wfΣ.
    ...
    eapply (red_red_context Σ wfΣ (Γ := exist Γ clΓ) (T := exist T clT)) in r as [t [r r']].
    2:exact rctx.
  ...
  Lemma ws_cumul_pb_red_context {pb} {Γ Γ'} {T U} :
    Σ ⊢ Γ ⇝ Γ' ->
    Σ ;;; Γ ⊢ T ≤[pb] U ->
    Σ ;;; Γ' ⊢ T ≤[pb] U.
  Proof using wfΣ.
    ...
    destruct (closed_red_red_context Hctx redl) as [lnf [redl0 redr0]].
    eapply red_context_eq_term_upto in leq. 2: exact Hctx.
    eapply closed_red_compare_term_l in leq as [? [? ?]]. 3:exact redr0.
    2:{ rewrite -(length_of Hctx). now eapply closed_red_open_right. }
    destruct (closed_red_red_context Hctx redr) as [rnf [redl1 redr1]].
  ...
  Lemma red_red_context_inv {Γ Δ : closed_context} {t : open_term Γ} {u} :
    red Σ Γ t u -> red_context Σ Δ Γ -> red Σ Δ t u.
  Proof using wfΣ.
    intros r rc.
    eapply red_red_ctx; tea; eauto with fvs.
  Qed.

  Lemma red_red_context_inv' {Γ Δ : context} {t u} :
    Σ ⊢ Δ ⇝ Γ ->
    Σ ;;; Γ ⊢ t ⇝ u ->
    Σ ;;; Δ ⊢ t ⇝ u.
  Proof using wfΣ.
    intros rc [onΓ ont r].
    move: (red_context_closed_left rc) => onΔ.
    eapply closed_red_context_red_context in rc.
    eapply red_red_ctx in r.
    econstructor; tea. all:eauto with fvs.
    all:try now rewrite (All2_fold_length rc).
  Qed.

  Lemma cumul_red_context_inv {pb} {Γ Γ' : context} {T U : term} :
    Σ ;;; Γ ⊢ T ≤[pb] U ->
    Σ ⊢ Γ' ⇝ Γ ->
    Σ ;;; Γ' ⊢ T ≤[pb] U.
  Proof using wfΣ.
    intros H Hctx.
    apply ws_cumul_pb_red in H as (v & v' & [redl redr leq]).
    epose proof (red_red_context_inv' Hctx redl).
    epose proof (red_red_context_inv' Hctx redr).
    eapply cored_context_eq_term_upto in leq. 2: exact Hctx.
  ...
  Lemma closed_red_eq_context_upto_l {pb Γ Δ} {u} {v} :
    ...
    ∑ v', Σ ;;; Δ ⊢ u ⇝ v' × eq_term Σ Σ Δ v v'.
  Proof using wfΣ.
    intros clΔ [onΓ onu r] c.
    unshelve epose proof (red_eq_context_upto_l _ _ _ c r) as [nf [red eq]]; eauto with fvs.
  ...
  Lemma closed_red_eq_context_upto_r {pb Γ Δ} {u} {v} :
    ...
    Σ ;;; Δ ⊢ u ⇝ v ->
    compare_context Σ pb Γ Δ ->
    ∑ v', Σ ;;; Γ ⊢ u ⇝ v' × eq_term Σ Σ Γ v v'.
  Proof using wfΣ.
    intros clΔ [onΓ onu r] c.
    unshelve epose proof (red_eq_context_upto_r _ _ _ c r) as [nf [red eq]]; eauto with fvs.
    symmetry in eq.
    exists nf. split; auto. split; eauto with fvs.
    now rewrite (All2_fold_length c).
  Qed.
  ...
  Lemma cumul_trans_red_leqterm {Γ : closed_context} {t u v : open_term Γ} :
    ...
    ∑ l o r, red Σ Γ t l ×
             red Σ Γ u o ×
             red Σ Γ v r ×
             leq_term Σ.1 Σ Γ l o × leq_term Σ.1 Σ Γ o r.
  Proof using wfΣ.
    intros X X0.
    intros.
    ...
    eapply red_eq_term_upto_univ_r in eq; try tc; tea; eauto with fvs.
    destruct eq as [t1 [red'0 eq2]].
    eapply red_eq_term_upto_univ_l in eq'; try tc; tea; eauto with fvs.
  ...
  Lemma cumul_pb_compare_context {pb pb' Γ Δ T U} :
    ...
    Σ ;;; Γ |- T <=[pb'] U.
  Proof using wfΣ.
    intros wfΓ wfT wfU eqctx cum.
    eapply cumul_alt in cum as (nf & nf' & redl & redr & eq).
    eapply eq_term_upto_univ_eq_ctx_rev in eq; tea.
  ...
  Proof using wfΣ.
    ...
    eapply eq_term_upto_univ_eq_ctx in eq; tea.
    destruct redl as [v' [redv' eqv']].
    destruct redr as [v'' [redv'' eqv'']].
    eapply cumul_alt. exists v', v''; repeat split; auto.
    transitivity nf.
  ...
  Lemma conv_eq_context_upto {Γ} {Δ} {T U} :
    ...
    Σ ;;; Δ |- T = U.
  Proof using wfΣ.
    intros ??? eqctx. symmetry in eqctx.
    eapply cumul_pb_compare_context; tea.
  Qed.
  ...
  Proof using wfΣ.
    intros eqctx cl cum.
    eapply ws_cumul_pb_red in cum as [nf [nf' [redl redr eq]]].
    ...
    transitivity nf.
    2: transitivity nf'; tas.
    all: eapply eq_term_compare_term; tas.
    now symmetry.
  Qed.
  ...
  Proof using wfΣ.
    ...
    now apply cumul_pb_compare_context.
  Qed.
  ...
  Lemma cumul_pb_alt_red_context {Γ : closed_context} {Γ' pb} {T U : open_term Γ} :
    Σ ;;; Γ |- T <=[pb] U ->
    red_context Σ Γ Γ' ->
    Σ ;;; Γ' |- T <=[pb] U.
  Proof using wfΣ.
    ...
    eapply red_context_eq_term_upto in leqvv'; tea.
    destruct (red_red_context _ _ redv Hctx) as [Tj [redTj redT'j]].
    destruct (red_red_context _ _ redv' Hctx) as [Uj [redUUj redU'j]].
  ...
  Lemma cumul_pb_alt_red_context_inv {Γ Γ' : closed_context} {pb} {T U : open_term Γ} :
    Σ ;;; Γ |- T <=[pb] U ->
    red_context Σ Γ' Γ ->
    Σ ;;; Γ' |- T <=[pb] U.
  Proof using wfΣ.
    ...
    eapply cored_context_eq_term_upto in leq; tea.
    pose proof (red_red_context_inv redl Hctx).
    pose proof (red_red_context_inv redr Hctx).
    apply cumul_alt.
    exists v, v'.
    split; auto.
  Qed.

  Lemma conv_alt_red_context {Γ : closed_context} {Γ'} {T U : open_term Γ} :
    Σ ;;; Γ |- T = U ->
    red_context Σ Γ Γ' ->
    Σ ;;; Γ' |- T = U.
  Proof using wfΣ.
    apply cumul_pb_alt_red_context.
  Qed.

  Lemma conv_alt_red_context_inv {Γ Γ' : closed_context} {T U : open_term Γ} :
    Σ ;;; Γ |- T = U ->
    red_context Σ Γ' Γ ->
    Σ ;;; Γ' |- T = U.
  Proof using wfΣ.
    apply cumul_pb_alt_red_context_inv.
  Qed.

  Lemma cumul_alt_red_context {Γ : closed_context} {Γ'} {T U : open_term Γ} :
    Σ ;;; Γ |- T <= U ->
    red_context Σ Γ Γ' ->
    Σ ;;; Γ' |- T <= U.
  Proof using wfΣ.
  ...
  Lemma cumul_alt_red_context_inv {Γ Γ' : closed_context} {T U : open_term Γ} :
    Σ ;;; Γ |- T <= U ->
    red_context Σ Γ' Γ ->
    Σ ;;; Γ' |- T <= U.
  Proof using wfΣ.
  ...
  Lemma ws_cumul_pb_red_context_inv {pb Γ Γ'} {T U} :
    Σ ⊢ Γ' ⇝ Γ ->
    Σ ;;; Γ ⊢ T ≤[pb] U ->
    Σ ;;; Γ' ⊢ T ≤[pb] U.
  Proof using wfΣ.
  ...
  Lemma closed_red_context_refl Γ : is_closed_context Γ -> Σ ⊢ Γ ⇝ Γ.
  Proof using Type.
  ...
  Proof using wfΣ.
    ...
    - destruct IHHctx as (Δ & Δ' & redl & redr & eq).
      destruct p.
      { apply (ws_cumul_pb_red_context redl) in eqt.
        ...
        transitivity v'; tas.
        eapply eq_term_compare_term.
        now eapply eq_term_upto_univ_eq_ctx_rev. }
      { apply (ws_cumul_pb_red_context redl) in eqb.
        ...
        intuition auto. constructor; auto. constructor; auto.
        1-2:eapply red_red_context_inv'; tea.
        constructor; auto. econstructor; eapply red_red_context_inv'; tea.
        eapply eq_term_upto_univ_eq_ctx_rev in eq', eq0'; tea.
        constructor => //. constructor; auto.
        cbn in *. transitivity v' => //.
        transitivity v0' => //.
        now eapply eq_term_compare_term. }
  Qed.
  ...
  Proof using wfΣ.
    ...
    eapply (ws_cumul_pb_red_context r) in H.
    destruct pb'; cbn in *.
    - eapply (ws_cumul_pb_eq_context_upto (symmetry elr)) in H. 2:eauto with fvs.
      now eapply (ws_cumul_pb_red_context_inv l) in H.
  ...
  Proof using wfΣ.
    ...
    apply ws_cumul_ctx_pb_red in Hctx => //.
    destruct Hctx as [Δ [Δ' [l [r elr]]]].
    eapply (ws_cumul_pb_red_context_inv r).
    destruct pb'; cbn in *.
    - eapply (ws_cumul_pb_red_context l) in H.
...
#[global] Hint Extern 4 (eq_term_upto_univ _ _ _ _ _ _ _) => reflexivity : pcuic.
