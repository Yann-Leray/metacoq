...
Lemma is_prop_bottom {Σ Γ T s s'} (Hcf : prop_sub_type = false) :
  wf Σ ->
  Σ ;;; Γ ⊢ T ≤ tSort s ->
  Σ ;;; Γ ⊢ T ≤ tSort s' ->
...
Proof using Type.
  ...
  unshelve eapply (leq_universe_prop_no_prop_sub_type _ _ _ _ leq'); eauto.
Qed.
...
Proof using Type.
  ...
  unshelve eapply (leq_universe_sprop_l _ _ _ leq'); eauto.
Qed.
...
Definition eq_univ_prop := eq_universe_ prop_sub_type (fun (_ : CS.t) _ _ => True) CS.empty.
...
Lemma eq_term_eq_term_prop_impl Σ t u :
  ...
  eq_term_prop Σ n t u.
Proof using Type.
  intros n eq. eapply eq_term_prop_impl; eauto.
...
Lemma cumul_sort_inv {Σ Γ u u'} {wfΣ : wf Σ} : 
  Σ ;;; Γ |- tSort u ~~ tSort u' ->
  eq_univ_prop u u'.
Proof using Type.
  intros equiv.
  ...
  subst.
  depelim eq. assumption.
Qed.

Lemma cumul_prop_props {Σ Γ u u'} {wfΣ : wf Σ} (Hcf : prop_sub_type = false): 
  Universe.is_prop u ->
  Σ ;;; Γ |- tSort u ~~ tSort u' ->
  Universe.is_prop u'.
Proof using Type.
  intros isp e.
  apply cumul_sort_inv in e.
  red in e. rewrite Hcf in e.
  destruct u, u' => //.
Qed.
...
Lemma univ_exprs_elements_map g s :
...
Lemma cumul_prop_cum_l {Σ Γ A T B} {wfΣ : wf Σ} : 
  Σ ;;; Γ |- A ~~ T -> 
  Σ ;;; Γ ⊢ A ≤ B ->
  Σ ;;; Γ |- B ~~ T.
Proof using Type.
...
Lemma cumul_prop_cum_r {Σ Γ A T B} {wfΣ : wf Σ} :
  Σ ;;; Γ |- A ~~ T -> 
  Σ ;;; Γ ⊢ B ≤ A ->
  Σ ;;; Γ |- B ~~ T.
Proof using Type.
...
Lemma cumul_prop_conv_l {Σ Γ A T B} {wfΣ : wf Σ} :
  Σ ;;; Γ |- A ~~ T -> 
  Σ ;;; Γ ⊢ A = B ->
  Σ ;;; Γ |- B ~~ T.
Proof using Type.
...
Lemma cumul_prop_conv_r {Σ Γ A T B} {wfΣ : wf Σ} :
  Σ ;;; Γ |- A ~~ T -> 
  Σ ;;; Γ ⊢ B = A ->
  Σ ;;; Γ |- B ~~ T.
Proof using Type.
...
Lemma eq_term_prop_mkApps_inv {Σ ind u args ind' u' args'} {wfΣ : wf Σ} :
...
Proof using Type.
  ...
  6:{ eapply inversion_App in X6 as (na' & A' & B' & hf & ha & cum); auto.
      specialize (X3 _ _ hf _ X7_1).
      specialize (X5 _ _ ha _ (eq_term_upto_univ_napp_leq X7_2)).
      eapply cumul_cumul_prop in cum; auto.
      transitivity (B' {0 := u0}) => //.
  ...
  - eapply inversion_Prod in X4 as (s1' & s2' & e' & Ha & Hb & Hs); auto.
    ...
    specialize (X3 _ _ Hb _ X5_2).
    eapply cumul_cumul_prop in Hs => //.
    eapply cumul_prop_trans; eauto.
    constructor; fvs. constructor.
    apply cumul_sort_inv in X1, X3.
    clear -X1 X3.
    all: destruct s2; cbn.
    all: destruct s2'; trivial.
    all: destruct s1; trivial.
    all: destruct s1'; trivial.

  - eapply inversion_Lambda in X4 as (s & B & e' & dom & bod & cum).
    specialize (X1 _ _ dom _ (eq_term_upto_univ_napp_leq X5_1)).
    specialize (X3 t0 B). 
  ...
  - eapply inversion_LetIn in X6 as (s1' & A & e' & dom & bod & codom & cum); auto.
    specialize (X1 _ _ dom _ (eq_term_upto_univ_napp_leq X7_2)).
    specialize (X3 _ _ bod _  (eq_term_upto_univ_napp_leq X7_1)).
  ...
  - eapply inversion_Case in X9 as (mdecl' & idecl' & isdecl' & indices' & data & cum); auto.
    ...
    destruct data.
    specialize (X7 _ _ scrut_ty _ (eq_term_upto_univ_napp_leq X10)).
  ...
  - eapply inversion_Proj in X3 as (u' & mdecl' & idecl' & cdecl' & pdecl' & args' & inv); auto.
    intuition auto.
    specialize (X2 _ _ a0 _ (eq_term_upto_univ_napp_leq X4)).
