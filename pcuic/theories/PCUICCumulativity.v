...
From MetaCoq.PCUIC Require Import PCUICAst PCUICAstUtils
     PCUICLiftSubst PCUICRelevance PCUICEquality PCUICUnivSubst
     PCUICReduction.
...
Notation " Σ ;;; Γ ⊢ t <===[ pb ] u" := (compare_term pb Σ Σ Γ t u) (at level 50, Γ, t, u at next level).

(** ** Cumulativity *)

Inductive cumulAlgo_gen `{checker_flags} (Σ : global_env_ext) (Γ : context) (pb : conv_pb) : term -> term -> Type :=
| cumul_refl t u : Σ ;;; marks_of_context Γ ⊢ t <===[ pb ] u -> Σ ;;; Γ |- t <=[pb] u
...
Lemma cumul_pb_alt `{cf : checker_flags} pb Σ Γ t u :
  Σ ;;; Γ |- t <=[pb] u <~> { v & { v' & (red Σ Γ t v * red Σ Γ u v' * compare_term pb Σ Σ (marks_of_context Γ) v v')%type } }.
Proof.
  split.
  - induction 1.
...
Lemma cumul_alt `{cf : checker_flags} Σ Γ t u :
  Σ ;;; Γ |- t <= u <~> { v & { v' & (red Σ Γ t v * red Σ Γ u v' * leq_term Σ Σ (marks_of_context Γ) v v')%type } }.
Proof. apply cumul_pb_alt. Qed.

Lemma conv_alt_red {cf : checker_flags} {Σ : global_env_ext} {Γ : context} {t u : term} :
  Σ;;; Γ |- t = u <~> (∑ v v' : term, (red Σ Γ t v × red Σ Γ u v') × 
    eq_term Σ (global_ext_constraints Σ) (marks_of_context Γ) v v').
Proof. apply cumul_pb_alt. Qed.
...
Lemma red_cumul_pb `{cf : checker_flags} {pb} {Σ : global_env_ext} {Γ t u} :
  red Σ Γ t u ->
  Σ ;;; Γ |- t <=[pb] u.
Proof.
...
Lemma red_cumul_pb_inv `{cf : checker_flags} {pb} {Σ : global_env_ext} {Γ t u} :
  red Σ Γ t u ->
  Σ ;;; Γ |- u <=[pb] t.
Proof.
  intros. apply clos_rt_rt1n in X.
...
Lemma red_cumul_inv `{cf : checker_flags} {Σ : global_env_ext} {Γ t u} :
  red Σ Γ t u ->
  Σ ;;; Γ |- u <= t.
Proof. apply red_cumul_pb_inv. Qed.


Lemma red_cumul_pb_cumul_pb `{cf : checker_flags} {pb} {Σ : global_env_ext} {Γ t u v} :
...
Proof. apply red_cumul_pb_cumul_pb. Qed.
...
Lemma red_cumul_pb_cumul_pb_inv `{cf : checker_flags} {pb} {Σ : global_env_ext} {Γ t u v} :
...
Proof. apply red_cumul_pb_cumul_pb_inv. Qed.
...
Lemma conv_cumul_pb {cf:checker_flags} pb Σ Γ t u :
  Σ ;;; Γ |- t = u -> Σ ;;; Γ |- t <=[pb] u.
Proof.
  intro H; destruct pb; tas.
  now apply conv_cumul2 in H.
Qed.
...
Lemma compare_term_compare_term_napp {cf:checker_flags} Σ ϕ napp Γ pb t t' :
  compare_term pb Σ ϕ Γ t t' -> 
  compare_term_napp pb Σ ϕ napp Γ t t'.
Proof.
  eapply compare_term_compare_term_napp. tc.
Qed.
...
Definition eq_termp_napp {cf:checker_flags} (pb: conv_pb) (Σ : global_env_ext) napp Γ :=
  compare_term_napp pb Σ Σ napp Γ.

Notation eq_termp pb Σ Γ := (compare_term pb Σ Σ Γ).
