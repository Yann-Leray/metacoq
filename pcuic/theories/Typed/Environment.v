...
From MetaCoq.Common Require Import Primitive Universes.
...
Module Type Term.
  ...
  Parameter Inline tRel : nat -> term -> sort -> term.
  Parameter Inline tSort : Sort.t -> term.
  Parameter Inline tProd : name -> term -> sort -> term -> sort -> term.
  (* Parameter Inline tLambda : name -> term -> sort -> term -> term -> sort -> term. *)
  (* Parameter Inline tLetIn : name -> term -> term -> sort -> term -> term -> sort -> term. *)
  (* Parameter Inline tInd : inductive -> Instance.t -> list term -> term. *)
...
Module Environment (T : Term).
  ...
  Definition vass na A s : context_decl :=
    {| decl_name := na ; decl_body := None ; decl_type := A ; decl_sort := s |}.

  (** Local (de Bruijn) let-binding *)

  Definition vdef na b B s : context_decl :=
    {| decl_name := na ; decl_body := Some b ; decl_type := B ; decl_sort := s |}.

  (** Local (de Bruijn) context *)

  Definition context := list context_decl.
  Definition mark_context := list relevance.
  Definition marks_of_context : context -> mark_context := fun l => List.map (fun d => relevance_of_sort d.(decl_sort)) l.
  ...
  Definition set_binder_name (na : name) (x : context_decl) : context_decl :=
    {| decl_name := na;
       decl_body := decl_body x;
       decl_type := decl_type x;
       decl_sort := decl_sort x |}.
  ...
  | cons d vs =>
    let ty s :=
      ...
      let ty := lift (context_assumptions vs + n) #|s| d.(decl_type) in
      (* Δ, smash_context vs, n , vs |- b' *)
      let ty := subst s 0 ty in
    ...
    | Some b =>
      ...
      b' :: s
    | None =>
      let s := extended_subst vs (S n) in
      tRel n (ty s) (decl_sort d) :: s
    end
  end.
  ...
  Definition fix_context (m : mfixpoint term) : context :=
    List.rev (mapi (fun i d => vass d.(dname) (lift i 0 d.(dtype)) d.(dsort)) m).
...
>>>
  ...
  *)
  ...
  Definition tImplSorts (sdom scodom : sort) : term :=
    tProd nAnon (tSort sdom) (Sort.super sdom) (tSort scodom) (Sort.super scodom).
  ...
  Definition primitive_invariants (p : prim_tag) (cdecl : constant_body) :=
    ...
    | primArray =>
      ...
      [/\ cdecl.(cst_type) = tImplSorts s s, cdecl.(cst_body) = None &
  ...
  (* Definition mkLambda_or_LetIn d t :=
    match d.(decl_body) with
    | None => tLambda d.(decl_name) d.(decl_type) d.(decl_sort) t
    | Some b => tLetIn d.(decl_name) b d.(decl_type) d.(decl_sort) t
    end. *)
  ...
  (* Definition mkProd_or_LetIn d t :=
    match d.(decl_body) with
    | None => tProd d.(decl_name) d.(decl_type) d.(decl_sort) t
    | Some b => tLetIn d.(decl_name) b d.(decl_type) d.(decl_sort) t
    end. *)
  ...
  Definition arities_context (l : list one_inductive_body) :=
    rev_map (fun ind => vass (nNamed ind.(ind_name)) ind.(ind_type) ind.(ind_sort)) l.
  ...
  Lemma nth_error_arities_context idecls i idecl :
    ...
    nth_error (arities_context idecls) i =
      Some {| decl_name := nNamed idecl.(ind_name);
              decl_body := None;
              decl_type := idecl.(ind_type);
              decl_sort := idecl.(ind_sort) |}.
  Proof using Type.
    intros hnth.
  ...
  | on_vass na t t' s :
    P t t' ->
    All_decls P (vass na t s) (vass na t' s)

  | on_vdef na b t b' t' s :
    P b b' ->
    P t t' ->
    All_decls P (vdef na b t s) (vdef na b' t' s).
  ...
  | on_vass_alpha na na' t t' s s' :
    P t t' ->
    All_decls_alpha P (vass na t s) (vass na' t' s')

  | on_vdef_alpha na na' b t b' t' s s' :
    P b b' ->
    P t t' ->
    All_decls_alpha P (vdef na b t s) (vdef na' b' t' s').
