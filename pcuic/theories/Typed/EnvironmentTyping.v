...
From MetaCoq.Common Require Import config Universes Primitive.
From MetaCoq.PCUIC Require Import TypedBasicAst Environment.
...
Module EnvTyping (T : Term) (E : EnvironmentSig T) (TU : TermUtils T E).
  ...
  Definition on_def_type (P : context -> judgment -> Type) Γ d :=
    P Γ (Judge None d.(dtype) (Some d.(dsort))).

  Definition on_def_body (P : context -> judgment -> Type) types Γ d :=
    P (Γ ,,, types) (Judge (Some d.(dbody)) (lift0 #|types| d.(dtype)) (Some d.(dsort))).
  ...
  Definition lift_wfu_term wf_term wf_univ (j : judgment) := option_default wf_term (j_term j) (unit : Type) × wf_term (j_typ j) × option_default wf_univ (j_sort j) (unit : Type).
  ...
  Definition lift_wfbu_term wfb_term wfb_univ (j : judgment) := option_default wfb_term (j_term j) true && wfb_term (j_typ j) && option_default wfb_univ (j_sort j) true.

  Definition lift_sorting checking sorting : judgment -> Type :=
    fun j => option_default (fun tm => checking tm (j_typ j)) (j_term j) (unit : Type) ×
                                ∑ s, sorting (j_typ j) s ×
                                  option_default (fun u => u = s) (j_sort j) True.
  ...
  Proof.
    ...
    destruct (reflect_option_default HPu (j_sort j)) => /= //; now constructor.
  Qed.
  ...
  Definition lift_sorting_extract {c s tm ty} (w : lift_sorting c s (Judge tm ty None)) :
    lift_sorting c s (Judge tm ty (Some w.2.π1))
    := (w.1, (w.2.π1; (w.2.π2.1, eq_refl))).

  Lemma lift_sorting_forget_univ {Pc Ps tm ty u} :
    lift_sorting Pc Ps (Judge tm ty u) ->
    lift_sorting Pc Ps (Judge tm ty None).
  Proof.
    intros (? & ? & ? & ?).
    repeat (eexists; tea).
  Qed.
  ...
  Lemma lift_sorting_ex_it_impl_gen {Pc Qc Ps Qs} {tm tm' : option term} {t t' : term} :
    forall tu: lift_sorting Pc Ps (Judge tm t None),
    let s := tu.2.π1 in
    match tm', tm with None, _ => unit | Some tm', Some tm => Pc tm t -> Qc tm' t' | _, _ => False end ->
    (Ps t s -> ∑ s', Qs t' s') ->
    lift_sorting Qc Qs (Judge tm' t' None).
  Proof.
  ...
  Proof.
    intro f.
    replace (judgment_map f j) with (Judge (option_map f (j_term j)) (f (j_typ j)) (option_map id (j_sort j))).
    2: unfold judgment_map; destruct (j_sort j) => //.
    intro tu.
  ...
  Lemma lift_typing_subjtyp {P Q Q' j} :
    ...
    lift_wfu_term Q Q' j.
  Proof.
    intros (Htm & s & Hty & e) HPQ HQQ'.
    repeat split.
    - destruct (j_term j) => //=. eapply fst, HPQ, Htm.
    - eapply fst, HPQ, Hty.
    - destruct (j_sort j) => //=. rewrite e. eapply HQQ', snd, HPQ, Hty.
  Qed.
  ...
  Section TypeLocal.
    ...
    | localenv_cons_abs Γ na t s :
        All_local_env Γ ->
        typing Γ (j_vass t s) ->
        All_local_env (Γ ,, vass na t s)

    | localenv_cons_def Γ na b t s :
        All_local_env Γ ->
        typing Γ (j_vdef b t s) ->
        All_local_env (Γ ,, vdef na b t s).
  ...
  Arguments localenv_cons_abs {_ _ _ _ _} _ _.

  Definition localenv_cons {typing Γ na bo t s} :
    All_local_env typing Γ -> typing Γ (Judge bo t (Some s)) -> All_local_env typing (Γ ,, mkdecl na bo t s)
  ...
  Definition All_local_env_snoc {P Γ decl} : All_local_env P Γ -> on_local_decl P Γ decl -> All_local_env P (Γ ,, decl) :=
    match decl with mkdecl na bo t s => localenv_cons end.
  ...
  Section All_local_env_rel.
    ...
    Definition All_local_rel_abs {P Γ Γ' na A s} :
      All_local_rel P Γ Γ' -> P (Γ ,,, Γ') (j_vass A s)
      -> All_local_rel P Γ (Γ',, vass na A s)
      := localenv_cons.

    Definition All_local_rel_def {P Γ Γ' na t A s} :
      All_local_rel P Γ Γ' ->
      P (Γ ,,, Γ') (j_vdef t A s) ->
      All_local_rel P Γ (Γ',, vdef na t A s)
      := localenv_cons.
  ...
  Section TypeLocalOver.
    ...
    | localenv_over_cons_abs Γ na t s
        (all : All_local_env (lift_sorting1 checking sorting) Γ) :
        All_local_env_over_sorting Γ all ->
        forall (tu : lift_sorting1 checking sorting Γ (j_vass t s))
          (Hs: sproperty Γ all _ _ tu.2.π2.1),
          All_local_env_over_sorting (Γ ,, vass na t s)
                              (localenv_cons_abs all tu)

    | localenv_over_cons_def Γ na b t s
        (all : All_local_env (lift_sorting1 checking sorting) Γ) :
        All_local_env_over_sorting Γ all ->
        forall (tu : lift_sorting1 checking sorting Γ (j_vdef b t s))
          (Hc: cproperty Γ all _ _ tu.1)
          (Hs: sproperty Γ all _ _ tu.2.π2.1),
          All_local_env_over_sorting (Γ ,, vdef na b t s)
  ...
  Section TypeCtxInst.
    ...
    | ctx_inst_ass na t s i inst Δ :
        typing Γ i t ->
        ctx_inst Γ inst (subst_telescope [i] 0 Δ) ->
        ctx_inst Γ (i :: inst) (vass na t s :: Δ)
    | ctx_inst_def na b t s inst Δ :
        ctx_inst Γ inst (subst_telescope [b] 0 Δ) ->
        ctx_inst Γ inst (vdef na b t s :: Δ).
  ...
  Section All_local_env_size.
    ...
    Fixpoint All_local_env_size_gen base Γ (w : All_local_env (lift_sorting1 checking sorting) Γ) : size :=
      match w with
      | localenv_nil => base
      | localenv_cons_abs Γ' na t s w' p => ssize _ _ _ p.2.π2.1 + All_local_env_size_gen base _ w'
      | localenv_cons_def Γ' na b t s w' p => csize _ _ _ p.1 + ssize _ _ _ p.2.π2.1 + All_local_env_size_gen base _ w'
      end.
...
Module Conversion (T : Term) (E : EnvironmentSig T) (TU : TermUtils T E) (ET : EnvTypingSig T E TU).
  ...
  | all_decls_alpha_vass {na na' : name} {t t' : term} {s s' : sort}
    (eqt : P pb t t') :
    All_decls_alpha_pb (vass na t s) (vass na' t' s')

  | all_decls_alpha_vdef {na na' : name} {b b' t t' : term} {s s' : sort}
    (eqb : P Conv b b') (* Note that definitions must be convertible, otherwise this notion
    of cumulativity is useless *)
    (eqt : P pb t t') :
    All_decls_alpha_pb (vdef na b t s) (vdef na' b' t' s').
...
Module GlobalMaps (T: Term) (E: EnvironmentSig T) (TU : TermUtils T E) (ET: EnvTypingSig T E TU) (C: ConversionSig T E TU ET) (L: LookupSig T E).
  ...
  Section GlobalMaps.
    ...
    (** For well-formedness of inductive declarations we need a way to check that a assumptions
      of a given context is typable in a sort [s]. We also force well-typing of the let-ins
      in any universe to imply wf_local. *)
    Fixpoint type_local_ctx Σ (Γ Δ : context) (s : sort) : Type :=
      match Δ with
      | [] => wf_sort Σ s
      | {| decl_name := na; decl_body := None; decl_type := t; decl_sort := s' |} as d :: Δ =>
          type_local_ctx Σ Γ Δ s × P Σ (Γ ,,, Δ) (j_decl d) × s = s'
      | {| decl_body := Some _; |} as d :: Δ =>
          type_local_ctx Σ Γ Δ s × P Σ (Γ ,,, Δ) (j_decl d)
      end.

    Fixpoint sorts_local_ctx Σ (Γ Δ : context) (us : list sort) : Type :=
      match Δ, us with
      | [], [] => unit
      | {| decl_name := na; decl_body := None; decl_type := t; decl_sort := s' |} as d :: Δ, u :: us =>
        sorts_local_ctx Σ Γ Δ us × P Σ (Γ ,,, Δ) (j_decl d) × u = s'
    ...
    Definition on_type Σ Γ T := P Σ Γ (Judge None T None).
    Definition on_type_sort Σ Γ T s := P Σ Γ (j_vass T s).
    ...
    *)
    ...
    Inductive positive_cstr_arg mdecl (Γ : context) : term -> Type :=
    (* | pos_arg_closed ty :
      closedn #|Γ| ty ->
      mdecl ;;; Γ |arg+> ty
  ...
  where " mdecl ;;; Γ |arg+> t " := (positive_cstr_arg mdecl Γ t) : type_scope.
    ...
    Inductive positive_cstr mdecl (i : nat) (Γ : context) : term -> Type :=
    ...
    *)
    ...
    Record on_constructor Σ mdecl i idecl ind_indices cdecl cunivs := {
      ...
      on_ctype : on_type_sort Σ (arities_context mdecl.(ind_bodies)) (cstr_type cdecl) idecl.(ind_sort);
      on_cargs :
        sorts_local_ctx Σ (arities_context mdecl.(ind_bodies) ,,, mdecl.(ind_params))
                      cdecl.(cstr_args) cunivs;
      on_cindices :
        ctx_inst (fun Γ t T => P Σ Γ (Judge (Some t) T None)) (arities_context mdecl.(ind_bodies) ,,, mdecl.(ind_params) ,,, cdecl.(cstr_args))
                      cdecl.(cstr_indices)
    ...
    Record on_proj mdecl (mind : kername) (i k : nat) (p : projection_body) (decl : context_decl) :=
      { on_proj_name : (* All projections are be named after a constructor argument. *)
          decl_name decl = nNamed p.(proj_name);
        (* on_proj_type :
    ...
    Definition on_constant_decl Σ d :=
      P Σ [] (Judge d.(cst_body) d.(cst_type) (Some d.(cst_sort))).
  ...
  Proof.
    ...
    { eapply ctx_inst_impl_gen; tea.
      { eexists; tea. }
      { intros; eapply H1, All_eta3; cbn. apply All_map_inv with (P:=fun P => P _ t T1) (f:=fun P Γ t T => snd P Σ Γ (Judge (Some t) T None)); tea. }
      { eapply All_map, All_impl; tea; intros *; destr_prod; destruct 1; cbn; tea. } }
    { move => ? H'.
...
Module DeclarationTyping (T : Term) (E : EnvironmentSig T) (TU : TermUtils T E)
  ...
  Definition isTypeSort `{checker_flags} (Σ : global_env_ext) (Γ : context) (t : term) (s : sort) :=
    on_type_sort (lift_typing typing) Σ Γ t s.

  Definition isType `{checker_flags} (Σ : global_env_ext) (Γ : context) (t : term) :=
    on_type (lift_typing typing) Σ Γ t.

  Lemma isTypeSort_isType `{checker_flags} (Σ : global_env_ext) (Γ : context) (t : term) (s : sort) : isTypeSort Σ Γ t s -> isType Σ Γ t.
  Proof. apply lift_sorting_forget_univ. Defined.
  #[export] Hint Resolve isTypeSort_isType : pcuic.

  Coercion isTypeSort_isType : isTypeSort >-> isType.
