...
From MetaCoq.PCUIC Require Import TypedAst TypedAstUtils TypedInduction.
Import Nat.
...
| assumption_context_vass na t s Γ : assumption_context Γ -> assumption_context (vass na t s :: Γ).
...
Lemma lift_rel_ge k n p ty s :
  p <= n ->
  lift k p (tRel n ty s) = tRel (k + n) (lift k p ty) s.
Proof.
  intros; simpl in |- *.
  now elim (leb_spec p n).
Qed.

Lemma lift_rel_lt k n p ty s :
  p > n -> lift k p (tRel n ty s) = tRel n (lift k p ty) s.
Proof.
  intros; simpl in |- *.
  now elim (leb_spec p n).
Qed.

Lemma subst_rel_lt u n k ty s :
  k > n -> subst u k (tRel n ty s) = tRel n (subst u k ty) s.
Proof.
  simpl in |- *; intros.
  elim (leb_spec k n); intro Hcomp; easy.
Qed.

Lemma subst_rel_gt u n k ty s :
  n >= k + length u -> subst u k (tRel n ty s) = tRel (n - length u) (subst u k ty) s.
Proof.
  simpl in |- *; intros.
...
Lemma subst_rel_eq u n i t p ty s :
  List.nth_error u i = Some t -> p = n + i ->
  subst u n (tRel p ty s) = (* tCast *) (lift0 n t)(*  (subst u n ty) s *).
Proof.
...
Ltac nth_leb_simpl :=
  ...
  | _ => lia || congruence || solve [repeat (f_equal; auto; try lia)]
  end.
...
Proof.
  ...
  - repeat nth_leb_simpl.
    all: try rewrite -> simpl_lift by easy.
    + (* f_equal; auto. *) f_equal; lia.
    + intros; f_equal. auto.
    + intros; f_equal. auto.
Qed.
...
Proof.
  ...
  - rewrite [subst N p _]/=. destruct (leb_spec p n); revgoals.
    { rewrite !subst_rel_lt; try easy. }
    destruct (nth_error_spec N (n - p)).
    + rewrite -> subst_rel_lt by lia.
      erewrite subst_rel_eq; cycle 1.
      by rewrite -> nth_error_map, e; reflexivity.
      by lia.
      cbn. f_equal; auto.
      now rewrite commut_lift_subst_rec //.
    + rewrite [subst P (p + _ + _) _]/=.
      destruct (leb_spec (p + length N + n0) n).
      destruct (nth_error_spec P (n - (p + length N + n0))).
      * erewrite subst_rel_eq by (tea; lia).
        cbn. f_equal; auto.
        relativize (p + _ + _).
        1: rewrite -> simpl_subst_rec; eauto; try lia.
        len; lia.
      * rewrite !subst_rel_gt; rewrite ?map_length; try lia. f_equal; auto; lia.
      * rewrite subst_rel_lt; try easy.
        rewrite -> subst_rel_gt; rewrite map_length.
        2: lia. f_equal. auto.
Qed.
...
Proof.
  ...
  - destruct (Nat.leb_spec k n).
    1: rewrite nth_error_nil Nat.sub_0_r.
    all: f_equal; auto.
Qed.
...
Lemma nth_error_extended_subst Γ k i :
  ...
  | Some decl =>
    let s := extended_subst (List.skipn (S i) Γ) (context_assumptions (List.firstn (S i) Γ) + k) in
    let f body := lift (context_assumptions (List.skipn (S i) Γ) +
      (context_assumptions (List.firstn i Γ) + k)) #|s| body in
    match decl_body decl with
    | None => Some (tRel (k + context_assumptions (List.firstn i Γ)) (subst0 s (f (decl_type decl))) (decl_sort decl))
    | Some body =>
      Some (subst0 s (f body))
    end
  | None => None
  end.
Proof.
  ...
  - rewrite skipn_S. cbn. destruct a as [na [] ty].
    ...
    + cbn.
      ...
      * cbn. rewrite IHΓ.
        ...
        all: cbn. all: lia_f_equal.
Qed.

Lemma lift_extended_subst' (Γ : context) k k' :
  extended_subst Γ (k + k') = map (lift0 k) (extended_subst Γ k').
Proof.
  ...
  - f_equal.
    { f_equal. rewrite (IHΓ (S k)) (IHΓ 1). len. rewrite distr_lift_subst simpl_lift //. 1,2:len; lia. rewrite map_map_compose map_lift_lift. lia_f_equal.  }
...
Lemma lift_extended_subst Γ k : extended_subst Γ k = map (lift0 k) (extended_subst Γ 0).
Proof.
  rewrite -[k]Nat.add_0_r lift_extended_subst' Nat.add_0_r //.
Qed.
