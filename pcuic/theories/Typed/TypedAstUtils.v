...
From MetaCoq.PCUIC Require Import TypedAst.
...
Ltac change_Sk :=
  repeat match goal with
    | |- context [(0 + ?y)] => progress change (0 + y) with y
    | |- context [(?y + 0)] => progress replace (y + 0) with y by now rewrite Nat.add_0_r
...
Definition isProd t :=
  match t with
  | tProd na A s B s' => true
  | _ => false
  end.

Inductive view_prod : term -> Type :=
| view_prod_prod na A s B s' : view_prod (tProd na A s B s')
| view_prod_other t : ~ isProd t -> view_prod t.

Equations view_prodc (t : term) : view_prod t :=
  view_prodc (tProd na A s B s') := view_prod_prod na A s B s';
...
| view_ind_tInd ind u args : view_ind (tInd ind u args)
| view_ind_other t : negb (isInd t) -> view_ind t.

Equations view_indc (t : term) : view_ind t :=
  view_indc (tInd ind u args) => view_ind_tInd ind u args;
  view_indc t => view_ind_other t _.

Inductive view_prod_sort : term -> Type :=
| view_prod_sort_prod na A s B s' : view_prod_sort (tProd na A s B s')
| view_prod_sort_sort s : view_prod_sort (tSort s)
...
Equations view_prod_sortc (t : term) : view_prod_sort t := {
  | tProd na A s B s' => view_prod_sort_prod na A s B s';
  | tSort s => view_prod_sort_sort s;
