...
Definition wf_rel r := ∑ s : Universe.t, relevance_of_sort s = r.
Definition wf_na (na: aname) := (wf_rel na.(binder_relevance)).

Definition wf_nactx := All2 (fun (x: aname) (y : context_decl) => eq_binder_annot x (decl_name y)).
...
| wf_tProd na t b : wf_na na -> wf t -> wf b -> wf (tProd na t b)
| wf_tLambda na t b : wf_na na -> wf t -> wf b -> wf (tLambda na t b)
| wf_tLetIn na t b b' : wf_na na -> wf t -> wf b -> wf b' -> wf (tLetIn na t b b')
...
| wf_tFix mfix k : All (fun def => [× wf_na def.(dname), wf def.(dtype) & wf def.(dbody)]) mfix ->
                  wf (tFix mfix k)
| wf_tCoFix mfix k : All (fun def => [× wf_na def.(dname), wf def.(dtype) & wf def.(dbody)]) mfix -> wf (tCoFix mfix k).
(* | wf_tInt i : wf (tInt i) *)
(* | wf_tFloat f : wf (tFloat f). *)
Arguments wf : clear implicits.
Derive Signature for wf.

Definition wf_def Σ def := [× wf_na def.(dname), wf Σ def.(dtype) & wf Σ def.(dbody)].
...
Definition wf_Inv Σ (t : term) : Type :=
  ...
  | tProd na t b => wf_na na * wf Σ t * wf Σ b
  | tLambda na t b => wf_na na * wf Σ t * wf Σ b
  | tLetIn na t b b' => wf_na na * wf Σ t * wf Σ b * wf Σ b'
...
Lemma term_wf_forall_list_ind Σ :
  forall P : term -> Type,
    ...
    (forall (na : aname), wf_na na -> forall (t : term), P t -> forall t0 : term, P t0 -> P (tProd na t t0)) ->
    (forall (na : aname), wf_na na -> forall (t : term), P t -> forall t0 : term, P t0 -> P (tLambda na t t0)) ->
    (forall (na : aname), wf_na na -> forall (t : term),
        P t -> forall t0 : term, P t0 -> forall t1 : term, P t1 -> P (tLetIn na t t0 t1)) ->
    ...
    (forall (m : mfixpoint term) (n : nat), All (fun x => [× wf_na x.(dname), P x.(dtype) & P x.(dbody)]) m -> P (tFix m n)) ->
    (forall (m : mfixpoint term) (n : nat), All (fun x => [× wf_na x.(dname), P x.(dtype) & P x.(dbody)]) m -> P (tCoFix m n)) ->
    (* (forall i, P (tInt i)) ->
    (forall f, P (tFloat f)) -> *)
...
Proof.
  ...
  - inv X0; auto.
    apply H16.
    red in X.
    eapply All_impl; [exact (All_mix X X1)|].
    intros ? [[] []]; split; intuition 1.
  - inv X0; auto.
    apply H17.
    red in X.
    eapply All_impl; [exact (All_mix X X1)|].
...
Definition wf_decl Σ d :=
  [× option_default (wf Σ) (decl_body d) True, wf Σ (decl_type d) & wf_na (decl_name d)].

Definition wf_decl_pred Σ : context -> term -> typ_or_rel_or_none -> Type :=
  (fun _ t T => wf Σ t * match T with
                        | Typ T => wf Σ T
                        | SortRel rel => wf_rel rel
                        | Sort => True
                        end).
