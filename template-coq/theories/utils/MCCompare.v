...
Module StringOT <: UsualOrderedType.
  ...
  Program Definition eqb_dec (l1 l2 : t) : {l1 = l2} + {l1 <> l2} := 
    match eqb l1 l2 with 
    | true => left _
    | false => right _
    end.
  Next Obligation.
    unfold eqb in Heq_anonymous.
    destruct (compare l1 l2) eqn:e; try congruence.
    now eapply compare_eq in e.
  Qed.
  Next Obligation.
    unfold eqb in Heq_anonymous.
    now rewrite compare_refl in Heq_anonymous.
  Qed.
